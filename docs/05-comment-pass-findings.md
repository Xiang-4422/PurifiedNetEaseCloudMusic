...
- **现象**：`RequestRepository` 直接 import `data/netease/api/src/dio_ext.dart`。
- **风险**：`core` 到 `data/netease` 的依赖方向不理想，替换 SDK 或扩展其他数据源时会牵动核心网络入口。
- **建议**：将请求代理抽象为 core port，由 data/netease 提供实现，或把该 repository 下沉到 netease data 层。
