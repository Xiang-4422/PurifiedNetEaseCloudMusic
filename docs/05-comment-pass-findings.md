...
### 13. 网易云 SDK bean 体量过大且注释来源不稳定

- **位置**：`lib/data/netease/api/src/api/**/bean.dart`、`lib/data/netease/api/src/api/play/cloud_entity.dart`。
- **现象**：SDK bean 文件包含大量接口字段，补齐 public 注释时只能按字段名做机械说明；这些文件更接近外部协议模型或 generated-like 代码。
- **风险**：后续如果重新生成或同步上游 SDK，声明级注释很容易被覆盖；字段语义也可能与真实接口文档存在偏差。
- **建议**：后续如果保留“不豁免 generated-like 代码”的规则，需要把注释生成流程纳入 SDK 同步脚本，或将 API 协议模型迁到明确的 generated 区域并用脚本补文档。

### 14. 复制组件的 public API 注释仍偏机械

- **位置**：`lib/widget/custom_zoom_drawer/src/**`、`lib/widget/cover_flow/coverflow.dart`。
- **现象**：这些组件更接近第三方拷贝或独立 UI 组件，public API 很多，补齐注释时主要保留字段和方法名语义。
- **风险**：注释满足 lint，但对真实使用约束、动画参数含义和组件边界的解释不足；后续改组件时仍需要读实现。
- **建议**：后续若这些组件继续作为项目内正式组件维护，应补一轮面向调用方的 API 文档；若只是 vendor 代码，应迁到明确 vendor 目录并建立同步策略。

### 15. 生成文件注释可能被后续工具覆盖

- **位置**：`lib/generated/assets.dart`、`lib/generated/json/base/json_convert_content.dart`、`lib/generated/json/base/json_field.dart`。
- **现象**：为满足 `public_member_api_docs`，本轮也给 generated-like 文件补了声明级注释。
- **风险**：后续资源或 JSON 生成工具再次运行时可能清空这些注释，导致 analyzer 重新失败。
- **建议**：后续把注释补齐纳入生成后处理步骤，或者在生成模板中直接输出 `///` 注释。

### 16. 大型 controller 与 presentation 文件仍只是补了注释

- **位置**：`lib/features/playback/player_controller.dart`、`lib/features/shell/shell_controller.dart`、`lib/features/playback/presentation/bottom_panel_view.dart`、`lib/features/playlist/playlist_widgets.dart`。
- **现象**：本轮只补声明级注释，未继续拆职责。部分文件仍承载较多状态同步、交互转发和展示组合逻辑。
- **风险**：注释能改善可读性，但不能替代职责下沉；后续新增需求如果继续加在这些文件中，仍可能回到“大 controller / 大 widget”状态。
- **建议**：后续架构治理按既有防回退规则执行，新增职责优先进入 application service、presenter 或更小的子 widget。
