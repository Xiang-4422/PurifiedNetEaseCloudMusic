...
以下决策作为当前阶段的默认前提，后续如需调整，必须先更新本文档：

- 本地数据库目标选型倾向 `Isar`
- 搜索默认遵循“本地优先、远程补充并回写”
- 本地音乐源不是附属功能，而是正式目标能力
- 离线下载不是可选增强，而是核心能力之一
- 允许在重构过渡期保留 `Hive + 正式本地数据库 + 旧页面/新入口` 并存
- 允许分链路渐进迁移，不要求单个页面一次性完成所有改造
...
- “缓存”和“下载”明确区分，且下载内容需要在 UI 上明确标记
- 离线能力采用手动开关模式，而不是只做自动降级
- 搜索结果采用单一结果流展示，本地优先命中，远程结果再补充刷新
- 本地音乐源是重要能力，但产品入口暂不与在线内容并列为双主入口
- 下载内容在正常内容流中可见，并带离线/已下载标记

补充说明：

- “单一结果流”只表示同一类结果不再拆分成本地分区和在线分区
...
- 搜索类入口至少要支持“本地命中即返回，远程作为补充和回写来源”的降级路径
...
- `Hive`：登录态、设置项、播放恢复状态、离线模式开关、极轻量配置缓存
- 正式本地媒体库：歌曲、歌词、歌单、歌单关系、播放历史、最近播放、下载记录、离线资源索引
- 内存缓存：仅作为过渡层或短时性能优化，不作为事实来源
...
- 无网络：禁止主动远程同步，应用仅依赖本地库和缓存继续工作

离线模式补充：

- 离线模式由用户手动开启或关闭
- 离线模式开启后，只展示本地可用内容并禁止主动远程请求
- 非离线模式下，仍然保留网络异常时的自动降级能力
...
- UI 可以渐进式迁移，不要求同一页面一次性全部改完，但新增或重构链路必须优先接入统一 repository 入口
- 像播放恢复、歌词缓存这类跨链路状态，也应逐步收口到稳定的存储入口，而不是散落在 controller 和 handler 中
...
- 过渡期的 source、媒体库与应用能力入口应优先通过启动阶段统一注册，避免继续由各 repository 在运行时各自 new 出分裂的数据视图
...
- 当共享内存实现已经无法覆盖跨重启场景时，应先切到可持久化的过渡实现，再进入正式数据库迁移，避免把“本地优先”长期停留在一次启动周期内
- 下载内容需要在正常内容流中可见，而不只停留在独立下载页
...
- 下载相关能力默认视为核心链路，需要与播放优先级和离线模式一起设计
...
- 离线模式是用户手动开启的显式模式，而不是单纯依赖网络状态自动切换
- 离线模式开启后，`LibraryRepository` 需要优先只返回本地媒体库中的数据，并阻断新的远程兜底读取
- 离线模式下的 UI 不应继续暴露明显依赖在线数据的入口；如果暂时无法提供本地等价能力，需要明确提示当前仅使用本地数据
- 离线模式的状态应通过独立存储入口管理，避免后续随着设置页或播放器重构再次把策略判断散落回页面层
...
为避免在重构期间引入过大回归，后续默认采用以下执行策略：

1. 优先改单条业务链路，不一次性重写整块页面
2. 优先建立统一入口，再逐步替换旧调用方
3. 优先把“本地优先”跑通，再补齐正式数据库实现
4. 优先把“多源协议”跑通，再新增第二个 source
