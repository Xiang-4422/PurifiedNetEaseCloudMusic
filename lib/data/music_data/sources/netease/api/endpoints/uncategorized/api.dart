...
mixin ApiUncategorized {
  ...
  DioMetaData countriesCodeListDioMetaData() {
    return DioMetaData(Uri.parse('http://interface3.music.163.com/eapi/lbs/countries/v1'), data: {}, options: joinOptions(encryptType: EncryptType.EApi, eApiUrl: '/api/lbs/countries/v1'));
  }
  ...
  Future<AudioMatchResultWrap> audioMatch(
    ...
    return Https.dioProxy.postUri(audioMatchDioMetaData(algorithmCode: algorithmCode, times: times, sessionId: sessionId, duration: duration, from: from, rawdata: rawdata)).then((Response value) {
  ...
  Future<UploadImageResult> uploadImage(String path, {int imgSize = 300, int imgX = 0, int imgY = 0}) async {
    ...
    await Https.dio.post("https://nosup-hz1.127.net/yyimgs/${res.result.objectKey}?offset=0&complete=true&version=1.0", data: formData, options: Options(contentType: 'image/jpeg', headers: {'x-nos-token': res.result.token}));
