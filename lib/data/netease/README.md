...
`remote/` 存放 repository 面向网易云远程能力的主要入口。它负责通过构造函数注入的 `NeteaseMusicApi` 调用接口，再通过 mapper 返回 domain entity、轻量结果记录或 data 层约定的快照。
...
- remote data source 通过构造函数注入的 `NeteaseMusicApi` 调用接口。
