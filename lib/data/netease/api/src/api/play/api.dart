...
mixin ApiPlay {
  ...
  /// 歌单收藏
  Future<ServerStatusBean> subscribePlayList(String pid,
      {bool subscribe = true}) {
    return Https.dioProxy
        .postUri(subscribePlayListDioMetaData(pid, subscribe: subscribe))
        .then((Response value) {
  ...
  DioMetaData subscribePlayListDioMetaData(String pid,
    ...
    var params = {'id': pid};
    return DioMetaData(
        joinUri('/weapi/playlist/${subscribe ? 'subscribe' : 'unsubscribe'}'),
        data: params,
        options: joinOptions());
  }
  ...
  Future<PlaymodeIntelligenceListWrap> playmodeIntelligenceList(
    ...
    return Https.dioProxy
        .postUri(playmodeIntelligenceListDioMetaData(
  ...
  DioMetaData playmodeIntelligenceListDioMetaData(
    ...
    var params = {
      ...
      'type': fromPlayAll ? 'fromPlayAll' : 'fromPlayOne',
  ...
  DioMetaData playListDetailDynamicDioMetaData(String playlistId,
    ...
    return DioMetaData(joinUri('/api/playlist/detail/dynamic'),
        data: params,
        options:
            joinOptions(userAgent: UserAgent.Pc, cookies: {'os': 'android'}));
  }
  ...
  /// 注 : 部分用户反馈获取的 url 会 403,hwaphon找到的 解决方案是当获取到音乐的 id 后，将 https://music.163.com/song/media/outer/url?id=id.mp3 以 src 赋予 Audio 即可播放
  /// [br] 码率,默认设置了 999000 即最大码率,如果要 320k 则可设置为 320000,其他类推
  Future<SongUrlListWrap> songDownloadUrl(List<String> songIds,
      {String level = 'exhigh'}) {
    return Https.dioProxy
        .postUri(songDownloadUrlDioMetaData(songIds, level: level))
        .then((Response value) {
      return SongUrlListWrap.fromJson(value.data);
    });
  }
  ...
  Future<AlbumListWrap> newestAlbumList() {
    return Https.dioProxy
        .postUri(_newestAlbumListDioMetaData())
        .then((Response value) {
      return AlbumListWrap.fromJson(value.data);
    });
  }
  ...
  Future<AlbumDetailWrap> albumDetail(String albumId) {
    return Https.dioProxy
        .postUri(_albumDetailDioMetaData(albumId))
        .then((Response value) {
  ...
  Future<AlbumDetailDynamicWrap> albumDetailDynamic(String albumId) {
    return Https.dioProxy
        .postUri(_albumDetailDynamicDioMetaData(albumId))
        .then((Response value) {
