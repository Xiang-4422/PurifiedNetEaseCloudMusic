...
mixin ApiPlay {
  String _normalizeImageUrl(String url) {
    ...
    final queryParameters = Map<String, String>.from(uri.queryParameters)..remove('param');
    return uri
        .replace(
  ...
  Future<MultiPlayListWrap> relatedPlayList(String songId) {
    return Https.dioProxy.getUri(relatedPlayListDioMetaData(songId)).then((Response value) {
      ...
      listWrap.playlists = [];
      try {
        const pattern = r'<div class="cver u-cover u-cover-3">[\s\S]*?<img src="([^"]+)">[\s\S]*?<a class="sname f-fs1 s-fc0" href="([^"]+)"[^>]*>([^<]+?)<\/a>[\s\S]*?<a class="nm nm f-thide s-fc3" href="([^"]+)"[^>]*>([^<]+?)<\/a>';
  ...
  DioMetaData songUrlDioMetaData(List<String> songIds, {int br = 999000}) {
    ...
    return DioMetaData(Uri.parse('https://interface3.music.163.com/eapi/song/enhance/player/url'), data: params, options: joinOptions(encryptType: EncryptType.EApi, cookies: {'os': 'pc'}, eApiUrl: '/api/song/enhance/player/url'));
  }
  ...
  DioMetaData _legacySongDownloadUrlDioMetaData(List<String> songIds, {String level = 'exhigh'}) {
    ...
    return DioMetaData(Uri.parse('https://interface.music.163.com/eapi/song/enhance/player/url/v1'), data: params, options: joinOptions(encryptType: EncryptType.EApi, eApiUrl: '/api/song/enhance/player/url/v1'));
  }
