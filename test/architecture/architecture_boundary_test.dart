  ...
  group('architecture boundary', () {
    ...
    test('top search panel keeps search context behind controller boundary', () {
      ...
      final violations = <String>[
        ...
        if (!controller.contains('final likedSongIds = _normalizedLikedSongIds(_likedSongIds());')) 'search controller does not normalize injected liked ids before building search context',
        if (!controller.contains('List<int> _normalizedLikedSongIds(List<int> likedSongIds)')) 'search controller does not define a liked ids normalizer',
        if (!controller.contains("return _normalizedLikedSongIds(likedSongIds).join(',');")) 'search controller signature does not reuse the liked ids normalizer',
