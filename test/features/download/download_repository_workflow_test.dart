...
void main() {
  group('DownloadRepository workflow', () {
    ...
    test('keeps existing local import resource instead of promoting it to download', () async {
      final localAudio = File('${tempDirectory.path}/local import.mp3');
      ...
      expect(track?.id, '1');
      expect(await taskDataSource.getTask('1'), isNull);
      expect(resourceWriter.promotedTrackIds, isEmpty);
      expect(resourceWriter.savedLocalPaths, isEmpty);
      expect(musicDataRepository.playbackUrlCallCount, 0);
    });
